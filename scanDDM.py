...
from segzero import get_obj_map
...
class scanDDM(object):
	...
	def simulate_scanpaths(self, n_observers, image=None, saliency_map=None, prompt=None):
		...
		# Resize on CPU to avoid CUDA shared memory issues with large images
		reshaped_saliency = T.Resize(size=15)(saliency_map.cpu()).squeeze().to(self.device)
